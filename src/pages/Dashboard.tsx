...
function buildDecisionSupport(reports: AnonymizedReportPayload[], feedback: AnonymousFeedbackRecord[], mode: SourceMode) {
  ...
  return {
    thisWeek: [
      ...
      mode === 'sample' ? 'Replace sample data with local pilot data before making operational decisions.' : mode === 'firebase' ? 'Confirm Firebase security rules, retention, and access owners before production use.' : 'Export local-only findings for the pilot owner; local browser data is not an institutional system of record.',
    ],
    ...
    dataQualityWarnings: [
      `Data source is ${mode}: ${mode === 'sample' ? 'synthetic sample data only' : mode === 'local' ? 'local browser feedback only' : 'Firebase option label only; project configuration and governance must be reviewed'}.`,
