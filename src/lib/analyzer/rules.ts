...
export const analyzerRules: AnalyzerRule[] = [
  ...
  { id: 'credentialOrOtpRisk', label: 'Credential/OTP risk', description: 'Mentions passwords, one-time codes, login codes, PINs, or 2FA details that should not be shared through messages.', weight: 13, pattern: /\b(password|OTP|one-time code|verification code|recovery code|login code|six digit code|code|2FA|PIN)\b/gi },
  { id: 'financialAccountRisk', label: 'Financial account/card risk', description: 'Mentions bank cards, bank accounts, transactions, refunds, charges, or fraud alerts.', weight: 11, pattern: /\b(card|debit card|credit card|bank card|bank account|transaction|payment method|account balance|refund|charge|recent charge|unauthorized transaction|fraud alert)\b/gi },
